...
if [ "${CLANG_BUILD}" = "INSTALL" -o "${CLANG_BUILD}" = "BUILD" ]; then
    ...
    SCYLLA_OPTS=(--mode=release --mode=debug --mode=dev --c-compiler="$STAGE1_BIN/clang" --compiler="$STAGE1_BIN/clang++" --enable-dpdk)

    echo "stage-0: build the bootstrapped compiler"

    cmake -B build -S llvm  "${USE_CURRENT_COMPILER[@]}" "${COMMON_OPTS[@]}" "${LLVM_TARGET}" -DLLVM_ENABLE_PROJECTS="clang;lld" -DLLVM_ENABLE_RUNTIMES="compiler-rt" -DCOMPILER_RT_BUILD_SANITIZERS=OFF -DCOMPILER_RT_DEFAULT_TARGET_ONLY=ON
    ninja -C build
    ...
    NEW_COMMON_OPTS=(-DLLVM_ENABLE_PROJECTS="clang;lld" -DLLVM_ENABLE_RUNTIMES="compiler-rt" -DLLVM_ENABLE_LTO=Thin -DCLANG_DEFAULT_PIE_ON_LINUX=OFF -DLLVM_BUILD_TOOLS=OFF)
    ...
    LLVM_PROFILE_FILE=$DIR/stage-1-${CLANG_ARCH}/build/profiles/default_%p-%m.profraw ninja build/{release,debug,dev}/scylla build/{release,debug,dev}/iotune
    ...
    rm -rf build build.ninja
    ./configure.py "${SCYLLA_OPTS[@]}"
    LLVM_PROFILE_FILE=$DIR/stage-1-${CLANG_ARCH}/build/profiles/csir-%p-%m.profraw ninja build/{release,debug,dev}/scylla build/{release,debug,dev}/iotune
    ...
    cd ../
    tar -cpJf ${DIR}/optimized_clang-${CLANG_ARCH}.tar.gz stage-1-${CLANG_ARCH}
elif [ "${CLANG_BUILD}" = "INSTALL_PREBUILT" ]; then
    cd "${DIR}"
