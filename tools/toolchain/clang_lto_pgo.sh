...
if [ "${CLANG_BUILD}" = "INSTALL" -o "${CLANG_BUILD}" = "BUILD" ]; then
    ...
    SCYLLA_OPTS=(--mode=debug --mode=release --mode=dev --mode=sanitize --mode=coverage --c-compiler="$STAGE1_BIN/clang" --compiler="$STAGE1_BIN/clang++" --enable-dpdk)
    ...
    LLVM_PROFILE_FILE=$DIR/stage-1-${CLANG_ARCH}/build/profiles/default_%p-%m.profraw ninja build/{debug,release,dev,sanitize,coverage}/scylla build/{debug,release,dev,sanitize,coverage}/iotune
    ...
    rm -rf build build.ninja
    ./configure.py "${SCYLLA_OPTS[@]}"
    LLVM_PROFILE_FILE=$DIR/stage-1-${CLANG_ARCH}/build/profiles/csir-%p-%m.profraw ninja build/{debug,release,dev,sanitize,coverage}/scylla build/{debug,release,dev,sanitize,coverage}/iotune
    ...
    cd ../
    tar -cpJf ${DIR}/optimized_clang-${CLANG_ARCH}.tar.xz stage-1-${CLANG_ARCH}
elif [ "${CLANG_BUILD}" = "INSTALL_PREBUILT" ]; then
    cd "${DIR}"
    tar -xpJf ${DIR}/optimized_clang-${CLANG_ARCH}.tar.xz
    cd stage-1-${CLANG_ARCH}
    sudo ninja -C build install
fi
