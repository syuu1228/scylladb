...
static void tcp_timestamps_sanity() {
    try {
        ...
        f.read(buf, 128);
        if (sstring(buf) == "0\n") {
            startlog.error("sysctl entry net.ipv4.tcp_timestamps is set to 0.\n"
                          "Scylla requires to set net.ipv4.tcp_timestamps=1");
            throw std::runtime_error("sysctl entry net.ipv4.tcp_timestamps is set to 0.");
        }
    } catch (const std::system_error& e) {
